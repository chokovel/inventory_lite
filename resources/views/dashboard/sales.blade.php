  ...
  <div class="card-body">
    ...
    @if(isset($sales))
        @foreach ($sales as $sale)
            <tr>
                <td class="text-left">{{ $sale->id }}<br>{{ $sale->created_at }}<br>{{ $sale->user->id }}</td>
                <td>{{ $sale->customer->name }}</td>
                <td>{{ $sale->product }}<br>{{ $sale->color }}<br>{{ $sale->size }}</td>
                <td>{{ $sale->qty }}<br>{{ $sale->total }}</td>
                <td class="td-actions text-right d-flex">
                <a href="{{ route('sales.edit', $sale->id) }}" class="btn btn-primary btn-round btn-sm">
